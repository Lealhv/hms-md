...
class SuppliersController
{
    ...
    public function create()
    {
        ...
        $stmt->bind_param("s", $data->SP_name);

        if ($stmt->execute()) {
            $new_id = $stmt->insert_id; // קבלת ה-ID החדש שנוצר
            echo json_encode(["message" => "Supplier created successfully", "SP_id" => $new_id]);
        } else {
            echo json_encode(["error" => "Failed to create supplier: " . $stmt->error]);
        }
    }
